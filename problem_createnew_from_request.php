...
$RequestID = $_GET["elementid"];
$RequestSubject = $_GET["requestsubject"];
$RequestCompanyID = $_GET["companyid"];
$NewProblemID = "";
$UserID = $_SESSION["id"];
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <div class="card">
                    <div class="card-header card-header-problems card-header-icon">
                        ...
                        <h4 class="card-title"> <?php echo _('Create problem based on ')._("request") ?> <?php echo $RequestID; ?></h4>
                    </div>
                    <div class="card-body">
                        ...
                        if (isset($_POST['create_problem'])) {
                            ...
                            $NewProblemID = createProblemFromRequest($ProblemName, $ProblemDescription, $ProblemCompany, $ProblemResponsible, $ProblemPriority, $ProblemStatus, $ProblemDeadline, $RequestID);
                        ...
                        <div class="form-group">
                            <form method="POST">
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <input type="text" class="form-control float-end" id="ProblemName" name="ProblemName" value="<?php echo _('Problem created from request') ?> <?php echo $RequestID; ?>">
                                </div>

                                <label class="control-label col-md-12 col-sm-12 col-xs-12"><?php echo _('Description') ?></label>
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <textarea id="ProblemDescription" name="ProblemDescription" rows="10" class="resizable_textarea form-control"><?php echo $RequestSubject; ?></textarea>
                                </div>
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <select id="ProblemResponsible" name="ProblemResponsible" class="form-control" required>
                                        <?php
                                        $sql = "SELECT ID, FullName FROM Requestusers WHERE Active=TRUE";
                                        $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
                                        while ($row = mysqli_fetch_array($result)) {
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <select id="ProblemPriority" name="ProblemPriority" class="form-control" required>
                                        <?php
                                        $sql = "SELECT ID, Name FROM Requestpriorities WHERE Active=TRUE";
                                        $result = mysqli_query($conn, $sql) or die('Query fail: ' . mysqli_error($conn));
                                        while ($row = mysqli_fetch_array($result)) {
                                            $Priority = $row['Name'];
