...
if (document.querySelector('.fixed-plugin')) {
  ...
  fixedPluginCloseButton.forEach(function (el) {
    el.onclick = function () {
      // Close the fixed plugin
      fixedPlugin.classList.remove("show");

      // Check for any open modals that are minimized
      var minimizedModals = document.querySelectorAll(".modal.show.minimized");
      minimizedModals.forEach(function (modal) {
        // Restore each minimized modal
...
if (iconNavbarSidenav) {
  iconNavbarSidenav.addEventListener("click", function (event) {
...
if (iconSidenav) {
  iconSidenav.addEventListener("click", function (event) {
    event.stopPropagation(); // Stop the event from bubbling up
    toggleSidenav();
  });
}
...
function navbarColorOnResize() {
  ...
  var referenceButtons = document.getElementById("referenceButtons");
