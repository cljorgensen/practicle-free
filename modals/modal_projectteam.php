<script>
  async function addUserToProject() {
    ...
    await $.ajax({
      type: 'GET',
      url: './getdata.php?addUserToProject',
      data: {
        UserID: UserID,
        ProjectID: ProjectID
      },
  ...
  async function removeUserFromProject() {
    ...
    await $.ajax({
      type: 'GET',
      url: './getdata.php?removeUserFromProject',
      data: {
        UserID: UserID,
        ProjectID: ProjectID
      },
...
<div class="modal fade" id="modalProjectTeam" data-bs-focus="false" role="dialog" aria-labelledby="ModalProjectTeamLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title font-weight-normal" id="ModalProjectTeamLabel"><?php echo $functions->translate('Project participants'); ?></h6>
                ...
                <div class="input-group input-group-static mb-4">
                  ...
                  <select class="form-control select2" id="teamusersforadd" name="teamusersforadd">
                    ...
                    $sql = "SELECT ID, CONCAT(users.Firstname,' ',users.Lastname,' ','(',users.username,')') AS FullName
                            FROM users 
                            WHERE ID IN (" . implode(",", $UsersArray) . ") AND ID NOT IN (SELECT UserID FROM project_users WHERE ProjectID = $ProjectID)
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="input-group input-group-static mb-4">
                  ...
                  <select class="form-control select2" id="teamusersforremoval" name="teamusersforremoval">
                    ...
                    $sql = "SELECT ID, CONCAT(users.Firstname,' ',users.Lastname,' ','(',users.username,')') AS FullName
                        FROM users WHERE ID IN (SELECT UserID FROM project_users WHERE ProjectID = $ProjectID)
                        ORDER BY users.Firstname ASC";
